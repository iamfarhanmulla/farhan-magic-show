    ...
    <section id="skills" className="section-padding" ref={ref}>
      <div className="max-w-5xl mx-auto">
        <motion.h2
          ...
          className="text-2xl md:text-3xl font-bold mb-14 flex items-center gap-3"
        >
        ...
        >
          {skills.map((skill) => (
            <div
              key={skill.name}
              className="skill-card opacity-0 glass rounded-xl p-4 md:p-5 flex flex-col items-center gap-3 hover:border-primary/50 hover:shadow-[0_0_20px_hsl(var(--primary)/0.15)] transition-all duration-300 group cursor-default"
            >
              <img
                src={skill.icon}
                alt={skill.name}
                className="skill-icon w-10 h-10 md:w-12 md:h-12 group-hover:scale-110 transition-transform duration-300"
              />
